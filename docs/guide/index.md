...
How consuming applications authenticate with Sentinel using `X-Service-Key` headers. Covers the dual-auth and service-only auth tiers, and key management via the admin panel.
